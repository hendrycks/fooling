...
def load_data():
    ...
    # convert to yuv
    X_train = x[0:50000, :, :, :]
    X_train = np.dot(X_train, yuv_from_rgb.T)
    # X_train[:, :, :, 0] = rescale(X_train[:, :, :, 0])
    # X_train[:, :, :, 1] = rescale(X_train[:, :, :, 1])
    # X_train[:, :, :, 2] = rescale(X_train[:, :, :, 2])
    ...
    X_test = x[50000:, :, :, :]
    X_test = np.dot(X_test, yuv_from_rgb.T)
    # X_test[:, :, :, 0] = rescale(X_test[:, :, :, 0])
    # X_test[:, :, :, 1] = rescale(X_test[:, :, :, 1])
    # X_test[:, :, :, 2] = rescale(X_test[:, :, :, 2])
...
def build_cnn(input_var=None, n=5):
    ...
    def residual_block(l, increase_dim=False, projection=False):
        ...
        if increase_dim:
            if projection:
                ...
                projection = batch_norm(ConvLayer(l, num_filters=out_num_filters, filter_size=(1, 1), stride=(2, 2),
                                                  nonlinearity=None, pad='same', b=None, flip_filters=False))
                block = NonlinearityLayer(ElemwiseSumLayer([stack_2, projection]), nonlinearity=rectify)
            else:
                # identity shortcut, as option A in paper
                identity = lasagne.layers.MaxPool2DLayer(l, (2, 2), stride=(2, 2))
                # identity = lasagne.layers.MaxPool2DLayer(l, (3,3), stride=(2,2), pad=1)
    ...
    l_in = batch_norm(InputLayer(shape=(None, 3, 32, 32), input_var=input_var))

    # first layer, output is 16 x 32 x 32
    l = batch_norm(ConvLayer(l_in, num_filters=16, filter_size=(3, 3), stride=(1, 1), nonlinearity=rectify,
...
def main(n=5, num_epochs=82, model=None):
    ...
    if model is None:
        ...
        for epoch in range(num_epochs):
            ...
            if (epoch + 1) % 10 == 0:
                np.savez('cifar10_deep_residual_model_yuv_unscaled' + str(epoch) + '.npz',
                         *lasagne.layers.get_all_param_values(network))
                pickle.dump(lasagne.layers.get_all_param_values(network), open('r32_yuv_unscaled.pkl', 'wb'))
                print('Saved')

        # dump the network weights to a file :
        np.savez('cifar10_deep_residual_model_yuv_unscaled_end.npz', *lasagne.layers.get_all_param_values(network))
        pickle.dump(lasagne.layers.get_all_param_values(network), open('r32_yuv_unscaled.pkl', 'wb'))
    else:
