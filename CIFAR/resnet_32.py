...
def load_data():
    ...
    # x -= pixel_mean
    # FOR A SIMPLE MIX OPTION IN THE FOOLER GENERATOR WE LEAVE MEAN SUBTRACTION TO BATCH NORM
...
def main(n=5, num_epochs=82, model=None):
    ...
    if model is None:
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % 10 == 0:
                np.savez('cifar10_deep_residual_model'+str(epoch)+'.npz', *lasagne.layers.get_all_param_values(network))
                pickle.dump(lasagne.layers.get_all_param_values(network), open('r32.pkl', 'wb'))
                print('Saved')

        # dump the network weights to a file :
        np.savez('cifar10_deep_residual_model_end.npz', *lasagne.layers.get_all_param_values(network))
        pickle.dump(lasagne.layers.get_all_param_values(network), open('r32.pkl', 'wb'))
    else:
